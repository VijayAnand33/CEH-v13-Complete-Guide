...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **AS-REP Roasting** | **AS-REP** (Auth Service Response) | User account password hash (RC4 / AES) | Account configured with `DONT_REQUIRE_PREAUTH` (Kerberos pre-authentication disabled) | Requests an AS-REP from the KDC without supplying timestamp proof; cracks the encrypted ticket offline.<br>`GetNPUsers.py corp.local/ -no-pass -usersfile users.txt`<br>`hashcat -m 18200 asrep.hashes rockyou.txt` | **4768** (TGT request without pre-auth validation) | Uncheck *"Do not require Kerberos preauthentication"* on all AD user accounts. |
| **Kerberoasting** | **TGS** (Ticket Granting Service Response) | Service Account NTLM Hash | Service Principal Name (`SPN`) assigned to a standard user account | Authenticated user requests a TGS ticket for an SPN service; dumps ticket from memory/network and cracks the service hash offline.<br>`GetUserSPNs.py corp.local/user:pass -request`<br>`hashcat -m 13100 kerb.hashes wordlist.txt` | **4769** (Spike in TGS requests with RC4 encryption `0x17`) | Migrate to **Group Managed Service Accounts (gMSA)** or enforce 25+ character complex passwords. |
| **Pass-the-Ticket (PtT)** | **TGT / TGS** | Active Kerberos Ticket in Memory (`.kirbi` / `.ccache`) | Unprotected LSASS memory space; lack of credential isolation | Dumps cached ticket from `lsass.exe` on a compromised host and injects it into the current logon session.<br>`sekurlsa::tickets /export`<br>`kerberos::ptt [ticket.kirbi]` | **4624** (Logon Type 9 / NewCredentials) | Enable **Windows Credential Guard** (VBS / LSAIso) and restrict local admin privileges. |
| **Silver Ticket** | **Forged TGS** (Service Ticket) | Target **Service Account NTLM Hash** (e.g., MSSQL, CIFS, HTTP) | Compromised service account password hash | Attacker crafts a valid Service Ticket directly, entirely bypassing communication with the KDC/Domain Controller.<br>`kerberos::golden /domain:corp.local /sid:S-1-5-21-... /target:fs01.corp.local /service:cifs /rc4:[SVC_NTLM] /user:Administrator /ptt` | Local member server logs; **zero** DC traffic (invisible to KDC logs) | Rotate the compromised service account's password; enforce AES Kerberos keys over RC4. |
| **Golden Ticket** | **Forged TGT** (Master Ticket Granting Ticket) | Root Domain **`krbtgt` Account NTLM Hash** | Complete domain compromise via DCSync, NTDS.dit extraction, or unconstrained delegation abuse | Forges an all-powerful TGT with arbitrary group memberships (e.g., Domain Admins / Enterprise Admins) valid for up to 10 years.<br>`kerberos::golden /domain:corp.local /sid:S-1-5-21-... /krbtgt:[KRBTGT_NTLM] /user:Administrator /id:500 /ptt` | **4768 / 4769** (Tickets with anomalous 10-year validity or non-existent account names) | **Reset the `krbtgt` password twice** (allowing full AD replication cycle between resets). |
| **DCSync** | **Directory Replication Service (DRS)** RPC | Replicating DC Machine Account or Domain Admin token | High AD privileges (`DS-Replication-Get-Changes` and `DS-Replication-Get-Changes-All`) | Pretends to be a Domain Controller over RPC and requests secret password hashes directly from the KDC without running code on the DC.<br>`lsadump::dcsync /domain:corp.local /user:krbtgt` | **4662** (Operation performed on AD root object for DRS replication GUIDs) | Audit and eliminate delegated replication rights on non-DC accounts; add admins to **Protected Users**. |
